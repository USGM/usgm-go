# Usglobalmail Go Library

[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-Built%20with%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=Usglobalmail%2FGo)

The Usglobalmail Go library provides convenient access to the Usglobalmail APIs from Go.
...
```go
response, err := client.Mails.Archive(...)
if err != nil {
...
```go
response, err := client.Mails.WithRawResponse.Archive(...)
if err != nil {
    return err
}
...
response, err := client.Mails.Archive(ctx, ...)
```
...
response, err := client.Mails.Archive(ctx, request, ...)
```
